...
- delete_protection: (Optional) Enable delete protection for this bucket. Defaults to `false`. When enabled, neither the bucket nor the objects it contains can be deleted by any caller, including applications using the S3 API. Set to false and apply before destroying the bucket.
